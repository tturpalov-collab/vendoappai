...
- **718 автоматов без продаж — это не поломка и не потеря, а запас.** Не подавать
  как проблему. Полезные метрики здесь: темп запуска (сколько точек открывается в
  месяц) и выручка на запущенный автомат (~4 400 AED/мес по августу 2026).
- Признак «включён, но не продаёт» — свежий `last_seen_at` при отсутствии продаж:
  такие машины стоит проверять отдельно, они уже на связи.
...
- **Пустой код ответа вниз не идёт** и держится на 700–1 000 в неделю. В августе
  2 509 из 2 612 (96 %) приходятся на одного клиента — `soco-br-of-majid-al-futtaim`.
  У него 63,9 % отказов было **в феврале, до аварии**; сейчас 42,6 % технических.
  Это интеграция или конфигурация эквайринга, а не сеть.
